...
LabelsGenerator::LabelsGenerator(const Repository& first_repository, const Repository& second_repository) {
	...
	//vector<vector<string>> - это одна метка - получаетс€
	//получаетс€ есть сет меток где изначально за строку метки надо вз€ть строку из первого репозитори€,
	//а за метку надо вз€ть набор таких строк из коммитов первого репа
	//далее - надо подготовить второй точно такой же сет дл€ второго репа
	//после чего, пока это возможно надо обогащать первый реп метками
	//как первый реп будет уже до конца обогащен нужно будет вз€ть соотвествие меток вершинам первого репа 
	//просто вз€в снова сделав 2 сета и опустошать оба от меток с меньшим количеством строк до меток - с большим 
	//и вставл€€ все это в first_labels, second_labels, с учетом инкапсул€ции LabelsGenerator
...
vector<LabelsGenerator::StringLabel> LabelsGenerator::GetStringLabels(const vector<HunkStringWithContext>& repository_hunks) {
	//тут отношение коммит - коммит, так как лэйбл здесь один большой дл€ всего коммита
