...
namespace TEST_GET_NUMBER_VERTEXES
{
	TEST_CLASS(TEST_GET_NUMBER_VERTEXES)
	{
	public:
		...
		TEST_METHOD(ZeroVertexes)
		{
			...
			Assert::IsTrue(graph.GetNumberVertexes() == 0);
		}
		...
		TEST_METHOD(OneVertex)
		{
			...
			Assert::IsTrue(graph.GetNumberVertexes() == 1);
		}
		...
		TEST_METHOD(SeveralVertexes)
		{
			...
			Assert::IsTrue(graph.GetNumberVertexes() == 3);
		}
